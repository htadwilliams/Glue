...
namespace Interceptor
{
    public class Input
    {
        ...
        public void SendKey(Keys key, KeyState state, uint information = 0)
        {
            Stroke stroke = new Stroke();
            ...
            keyStroke.Information = information;
        ...
        /// </summary>
        public void SendKey(Keys key, uint information = 0)
        {
            SendKey(key, KeyState.Down, information);

            if (KeyPressDelay > 0)
                Thread.Sleep(KeyPressDelay);

            SendKey(key, KeyState.Up, information);
        }
        ...
        public void SendText(string text, uint information = 0)
        {
            foreach (char letter in text)
            {
                ...
                if (tuple.Item2 == true) // We need to press shift to get the next character
                    SendKey(Keys.LeftShift, KeyState.Down, information);

                SendKey(tuple.Item1, information);

                if (tuple.Item2 == true)
                    SendKey(Keys.LeftShift, KeyState.Up, information);
            }
        }
        ...
        public void SendLeftClick(ushort information = 0)
        {
            SendMouseEvent(MouseState.LeftDown, information);
            Thread.Sleep(ClickDelay);
            SendMouseEvent(MouseState.LeftUp, information);
        }

        public void SendRightClick(ushort information = 0)
        {
            SendMouseEvent(MouseState.RightDown, information);
            Thread.Sleep(ClickDelay);
            SendMouseEvent(MouseState.RightUp, information);
        }

        public void ScrollMouse(ScrollDirection direction, ushort information = 0)
        {
            switch (direction)
            { 
                case ScrollDirection.Down:
                    SendMouseEvent(MouseState.ScrollDown, information);
                    break;
                case ScrollDirection.Up:
                    SendMouseEvent(MouseState.ScrollUp, information);
                    break;
            }
        }
